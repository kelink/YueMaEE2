...
public class UserController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/getRegisterCode", method = {RequestMethod.POST,RequestMethod.GET})
	@ResponseBody
	public String getRegisterCode(@RequestParam(C.ParamsName.PHONE_NUM) String phoneNum,
	 ...
	 /**
		...
		@ResponseBody
		public String register(@RequestParam(C.ParamsName.PHONE_NUM) String phoneNum,
				@RequestParam(C.ParamsName.PASSWORD) String pwd,
			...
			if (phoneNum == null || phoneNum.equals("") || pwd == null
					...
					|| registerCode.equals("")) {
				message.setCode(C.ResponseCode.ERROR);
				message.setMessage(C.ResponseMessage.REGIST_ERROR_PARMANTS);
				message.setResultMap(null);
			} else {
				//验证注册码
				...
				if (seeesionTime==null||phoneNum == null || phoneNum.equals("")) {
					...
					if (System.currentTimeMillis() - Long.parseLong(parms[0]) > (1000 * 60)) {
						...
						if (!registerCode.equals(parms[1])) {
							message.setCode(C.ResponseCode.ERROR);
							message.setMessage(CResponse.Message.REGIST_CODE_ERROR);
							message.setResultMap(null);
						}else {
							if (userService.getUserByPhone(phoneNum)!=null) {
								message.setCode(C.ResponseCode.ERROR);
